...
struct scanStruct						// Graph structure declaration for scans
{
	...
	char		fileName[7];			// Added in Version 03.2 for labelling the plot
} scan;
...
//		analogReference ( DEFAULT );			// 2.56V internal reference
//		vRef = 5.0;								// Really used in AD8307 computations
	#endif
 ...
 */
			...
			{
				tempLowerBandEdge = GetBandEdge ( activeBandBottom, LOWER );
 ...
 */
	...
	tempFreq  = GetBandEdge ( activeBandIndex, UPPER );		// Only seems to work right
	tempFreq += GetBandEdge ( activeBandIndex, LOWER );		// When strung out into multiple
	tempFreq /= 2L;											// Statements like this!
 ...
 */
	...
	while ( digitalRead ( SWITCH ))						// While encoder switch not operated
	{
		if ( encoderDirection != DIR_NONE )				// If encoder changed
		{
    		slope += ( float ) clickCount * 0.0001;		// Adjust the AD8307 xlope factor
	...
	for ( address = 0; address < EEPROM_NEXT_ADDR; address +=2 )	// All 2 byte integers (except FC & slope)
	{
		...
		switch ( address )
		{
			...
			case SLOPE_CALIBRATION:
				...
				FormatFloat ( floatValue, 1, 4, buff );
				Serial.print ( buff );
				break;
...
void Splash ()
{
	...
	PaintText ( 2, SCREEN_C,  5, 0, ERR,     CENTER, "Upgraded Software Version 3.7.1" );
